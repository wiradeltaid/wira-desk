...
- **Rule:** The Hook Thread is solely responsible for anti-macro throttle (reject inputs <50ms apart). It translates valid keypresses (and, since `FR-30`/`AD-15`, valid mouse inputs) into a `u8` command enum — `1`=Cycle through `19`=ShowDesktop cover every use case this spine describes (see `inventory-api.md`'s Ring Buffer Command row for the full list); ten further values (`20`-`29`) drive the visual switcher, shipped in code but tracked as open corpus debt (`SPEC-14`) rather than described here — before writing to the ring buffer. The Worker Thread never performs input validation — it only executes commands. Wire values are **extended, never renumbered**: a value already assigned keeps its meaning permanently, because a command sitting in the ring buffer carries only the number. Any value outside the assigned set decodes to `Nop` rather than being treated as an error.
