...
| **Installer Lifecycle & Downgrade Prevention** (Inno Setup / 64-bit HKLM) | Polled shutdown waits up to 6s for daemon WM_CLOSE then taskkill /F, bounded close for Settings without /F. | AppId uninstall key absent in 64-bit HKLM (first install proceeds cleanly). | Installed DisplayVersion newer than candidate or malformed/blank. | Downgrade or malformed version rejected before extraction with informative explanation; silent install exits non-zero without modal dialog; surviving processes abort install cleanly before file extraction (SPEC-21, DEF-23). | Setup log records installed vs candidate version or process termination error string. |
