...
9. Excluding a disabled action's chord from the match set entirely when building `Chords` from a config snapshot (`FR-29`, `BR-9`). A disabled action never reaches step 5 above: there is no configured chord for the hook to translate, so a keystroke that would have matched it falls through `match_shortcut` unmatched and is passed to `CallNextHookEx` the same as any chord Wira Desk was never configured to claim.
...
### Outbound Signals
- `ring::push(u8)`: Pushes `Command` opcode (`Cycle = 1`, `SnapLeft = 2`, `SnapRight = 3`, `SnapMaximize = 4`, `OverlappingStack = 5`, `SnapTop = 6`, `SnapBottom = 7`, `MoveToNextMonitor = 8`, `SnapPercentLeft = 9`, `SnapPercentRight = 10`, `SnapPercentTop = 11`, `SnapPercentBottom = 12`, `SnapThirdLeft = 13`, `SnapThirdMiddle = 14`, `SnapThirdRight = 15`, `NextVirtualDesktop = 16`, `PrevVirtualDesktop = 17`, `TaskView = 18`, `ShowDesktop = 19`) into the lock-free ring buffer. Opcodes 9–15 were added under `FR-26`/`FR-27` (`SPEC-1`) and are recorded here now, having been missed at the time — the design record for that spec landed only in `LC-arrangement-engine.md`, not here, though this component is what actually pushes them onto the wire. Opcodes 16–19 are the mouse-navigation actions (`FR-30`, `UC-13`, below). `shared::Command` also defines ten further values (`20`–`29`) driving the visual switcher — pushed by this same path but not yet described by this component's design record; `SPEC-14` tracks that as open corpus debt.
...
- **Disabled-action exclusion (`FR-29`):** Verified. `crates/daemon/src/hook.rs`'s `Chords` struct is `Option<Shortcut>` per field, and a chord absent from the struct takes no part in `match_shortcut` or `unbind_duplicates` — both `disabled` and `unbound` collapse to the same `None` representation at this layer. `Chords::from_config` builds every field behind its own `*_enabled` check, so a disabled action's chord never enters the struct in the first place — confirmed by `hook::tests::a_disabled_action_is_absent_from_chords` and `hook::tests::a_disabled_actions_chord_falls_through_to_call_next_hook_ex`.
