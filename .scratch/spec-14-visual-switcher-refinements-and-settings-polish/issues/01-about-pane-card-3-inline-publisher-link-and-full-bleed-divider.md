...
- [x] The standalone "Publisher website (wiradigital.id)" row and its adjacent `CardDivider` are gone from Card 3.
- [x] The attribution line reads `An open-source utility by Wira Digital Indonesia ↗ • Licensed under GPL-3.0`, with only "Wira Digital Indonesia" and the icon styled as a link.
- [x] Hovering the link changes both the text and the `OpenLinkIcon` to `Palette.accent_hover` and shows a pointer cursor; the two plain parts do not change.
- [x] Clicking the link invokes `root.open_publisher_url()` and opens `https://wiradigital.id`.
- [x] The link carries `accessible-role: button` and an accessible label naming the destination; the plain parts carry none.
- [x] Card 3's `VerticalLayout` uses `padding: 0px` with per-section padding, and its remaining dividers span the full card width — verified against the Updates card, not against a geometry override on `CardDivider`.
- [x] The About pane's minimum content width is **named as a number** taken from `main_window.slint`, and the attribution run is asserted to fit inside it minus Card 3's horizontal padding — or the run wraps, and the wrap is asserted.
- [x] At that width the pane shows **no** horizontal scrollbar, and neither does it at the width ticket 02's hold-delay caption is asserted against.
- [x] Removing the standalone row does not leave an orphaned `CardDivider` with no section beneath it.
- [x] Keyboard focus reaches the inline link, and the pane's focus order has no gap where the removed row used to be (`app::tests::focus_order_has_no_duplicates` still passes).
- [x] Automated tests cover: the inline link invokes `open_publisher_url`, the standalone row is absent, and the attribution line does not force horizontal overflow.
