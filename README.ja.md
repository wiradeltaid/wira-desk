...
PowerToys の Window Hopper や周辺機器メーカーの専用ツールは、バックグラウンドで複数のプロセスを走らせ 150〜500 MB ものメモリを消費します。Wira Desk は単一の軽量ネイティブデーモンとして動作し、メモリ消費量は約 4.0 MB（目標予算 5 MB 以下）で、テレメトリや外部通信は一切行いません。
