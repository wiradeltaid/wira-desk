...
pub mod tests {
    ...
    #[derive(Clone)]
    pub struct FailingThumbnailSink {
        pub fail_at_source: HWND,
        pub registered: Arc<Mutex<Vec<HWND>>>,
    }

    impl ThumbnailSink for FailingThumbnailSink {
        fn register(&mut self, _destination: HWND, source: HWND) -> Result<isize, i32> {
            if source == self.fail_at_source {
                Err(-1)
            } else {
        ...
        fn unregister(&mut self, handle: isize) {
            ...
            reg.retain(|&s| s != handle);
        }
    }

    #[test]
    fn a_failed_registration_degrades_one_card_not_the_overlay() {
        let registered = Arc::new(Mutex::new(Vec::new()));
        let sink = FailingThumbnailSink {
            fail_at_source: 3, // chosen index fails
            registered: Arc::clone(&registered),
        };
        let sink_arc: Arc<Mutex<dyn ThumbnailSink>> = Arc::new(Mutex::new(sink));
        let mut handles = Vec::new();
        for i in 1..=5 {
            let mut s = sink_arc.lock().unwrap();
            if let Ok(handle_val) = s.register(100, i) {
                handles.push(DwmThumbnailHandle::new(handle_val, Arc::clone(&sink_arc)));
            }
        }
        // 4 succeeded, 1 degraded
        assert_eq!(handles.len(), 4);
        assert_eq!(registered.lock().unwrap().len(), 4);
        assert!(!registered.lock().unwrap().contains(&3));

        drop(handles);
        // All succeeded registrations clean up on dismissal
        assert_eq!(registered.lock().unwrap().len(), 0);
    }
}
